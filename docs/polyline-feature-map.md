...
- `app/src/drawable/2d/polygon2d.ts` — **`Polygon2D`** (the line/polygon). Key symbols:
  ...
  polyline.
- **Endpoint join / merge** (drag an open line's START or END vertex onto
  another endpoint within 15 screen px): `Label2DList.findNearestEndpoint`
  returns an `EndpointSnapCandidate {polyline, isStart, mergeable}` for candidate
  lines with matching primary category (`category[0]`) or self-close;
  `Polygon2D.onMouseMove` (RESHAPE) snaps the dragged coord onto the target
  point and shows `drawSnapIndicator`; `onMouseUp` calls `mergeWith`.
  Different-category endpoints do NOT snap or join.
  Indicator: solid green halo + white centre = merge.
  Merge deletes the target via `_mergedOut` → `isValid()=false`
